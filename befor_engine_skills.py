...
class SkillEngineMixin:
    ...
    def apply_skill(self, skill, caster, frame, is_full_burst):
        print("★コード更新確認: 新しいファイルが読み込まれています★") 
        # ...
        ...
        if targets:
            ...
            # デバッグ用：もし重複があったらここで報告されるはず
            # print(f"DEBUG: Targets cleaned. Count: {len(targets)}")
        # ▲▲▲ 修正ここまで ▲▲▲
        if not targets and skill.effect_type == 'damage': targets.append(caster)
        
        # ▼▼▼ 追加: ターゲットリストの強制重複排除 ▼▼▼
        # これを入れることで、もし計算過程でターゲットが重複しても強制的に1人になります
        if targets:
            seen_names = set()
            unique_targets = []
            for t in targets:
        ...
        for target in targets:
            if skill.effect_type == 'ammo_charge' or skill.effect_type == 'refill_ammo':
                ...
                for target in targets:
                    # ▼▼▼【ここに追加】最終防衛ライン：同一フレーム・同一ターゲットへの重複適用をブロック ▼▼▼
                    # どんな理由でスキルが連打されても、ターゲットが重複していても、ここで全て無効化します
                    apply_guard_key = f"FINAL_GUARD::{frame}::{target.name}::{skill.name}"
                    
                    # すでにこのフレームで、このキャラに、このスキルを適用済みなら、ログも出さずにスキップ
